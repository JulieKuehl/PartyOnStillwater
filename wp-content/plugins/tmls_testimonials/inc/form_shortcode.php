<?php
	...
	function tmls_form_shortcode($atts, $content=null) {  
		...
		if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["tmls_form_submit"]) {
			...
			if (empty($_POST["tmls_form_companyWebsite"])) {
				...
				$tmls_companyWebsite = str_replace('http://','',$tmls_companyWebsite);
				$tmls_companyWebsite = str_replace('https://','',$tmls_companyWebsite);
		...
		if($form_width < 350 ) {
			$html.='<form autocomplete="off" id="tmls_form" method="post" action="'.$_SERVER['REQUEST_URI'].'#tmls_formmessage" class="tmls_form tmls_form_onecolumn" style="width:'.$form_width.'px;" >';
		}
		else {
			$html.='<form autocomplete="off" id="tmls_form" method="post" action="'.$_SERVER['REQUEST_URI'].'#tmls_formmessage" class="tmls_form" style="width:'.$form_width.'px;" >';
		...
		if($is_generate_shortcode_page_form == true) {
			$html.=	'<h3>Success Message:</h3>
					...
					<div id="tmls_formmessage" class="tmls_form_error_message" style="'.$tmls_validationmessage_style.'">
						...
						<span>'.$invalidcompanywebsite_message.'</span>
						<span>'.$captchaanswerrequired_message.'</span>
